...
export default async function EventPosts(callback) {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            let EventPostsJson = fs.readJsonSync(path.join(__dirname, './database/EventPosts.json'));
            ...
            if (data?.latest_posts[0]?.post_number === 1 && EventPostsJson.includes(id) === false) {
                ...
                fs.writeJsonSync(path.join(__dirname, './database/EventPosts.json'), EventPostsJson);
            }
